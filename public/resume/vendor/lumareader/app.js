(function () {
  ...
  const DEFAULT_TOOLBAR_VISIBILITY = Object.freeze({language:false,readingMode:false,source:false,media:false,textSize:false,exportPdf:false});
  ...
  function toolbarVisibilityTargets(){return{language:[languageToggleEl.closest(".language-control")],readingMode:[readingModeControlEl],source:[$("#source-view")],media:[$("#media-view")],textSize:[$("#font-down"),$("#font-up")],exportPdf:[$("#export-pdf")]}}
  function applyToolbarVisibility(){document.body.classList.toggle("resume-minimal-toolbar",["language","readingMode","textSize","exportPdf"].every(key=>state.toolbarVisibility[key]===false));const targets=toolbarVisibilityTargets();Object.entries(targets).forEach(([key,elements])=>elements.filter(Boolean).forEach((element)=>{if(state.toolbarVisibility[key]===false)element.dataset.userHidden="true";else delete element.dataset.userHidden;}));document.querySelectorAll("[data-toolbar-visibility]").forEach((input)=>{input.checked=state.toolbarVisibility[input.dataset.toolbarVisibility]!==false;});}
